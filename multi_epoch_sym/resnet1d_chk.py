...
class BasicBlock_Bottle(nn.Module):
    ...
    def forward(self, x):
        ...
        out = checkpoint(self.conv1, x, preserve_rng_state = True, use_reentrant=False)
        out = self.bn1(out)
        out = checkpoint(self.relu, out, preserve_rng_state = True, use_reentrant=False)
        out = self.drop(out)

        out = checkpoint(self.conv2, out, preserve_rng_state = True, use_reentrant=False)
        out = self.bn2(out)
        out = checkpoint(self.relu, out, preserve_rng_state = True, use_reentrant=False)
        out = checkpoint(self.conv3, out, preserve_rng_state = True, use_reentrant=False)
        out = self.bn3(out)
