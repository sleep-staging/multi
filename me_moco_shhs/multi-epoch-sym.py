...
def main():
    ...
    wb = wandb.init(
            project="EPF-Me-Moco-SHHS",
            notes="single-epoch, symmetric loss, 1000 samples, using same projection heads and no batch norm, original simclr",
            save_code=True,
            entity="sleep-staging",
            name="Multi Epoch Moco SHHS Epoch 7 T=0.5",
        )
    wb.save('multi/me_moco_shhs/*.py')
    wb.watch([q_encoder, k_encoder],log='all',log_freq=500)

    Pretext(q_encoder, k_encoder, m, optimizer, n_epochs, criterion, pretext_loader, test_records, wb, device, SAVE_PATH, BATCH_SIZE)
